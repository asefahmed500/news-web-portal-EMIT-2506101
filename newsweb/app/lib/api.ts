...
export const api = {
  ...
  getNews: (id: number | string) => apiFetch<NewsItem>(`/news/${id}`),
  createNews: (payload: Omit<NewsItem, "id">) =>
    apiFetch<NewsItem>("/news", { method: "POST", body: JSON.stringify(payload) }),
  patchNews: (id: number | string, patch: Partial<NewsItem>) =>
    apiFetch<NewsItem>(`/news/${id}`, {
      method: "PATCH",
      body: JSON.stringify(patch),
    }),
  deleteNews: (id: number | string) => apiFetch<void>(`/news/${id}`, { method: "DELETE" }),
};
